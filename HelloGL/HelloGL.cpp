...
void HelloGL::InitObjects()
{
	...
	OBJMesh* objMesh = OBJLoader::LoadOBJ((char*)"suzan.obj");	
	...
	bitMapImage->LoadBitMap((char*)"Monke.bmp", (char*)"Monke.raw");

	texture->Load((char*)"Monke.raw", 256, 256);

	for (int i = 0; i < 30; i++)
	{
		objects[i] = new Suzanne(objMesh, texture,((rand() % 400) / 10.0f) - 20.0f, ((rand() % 200) / 10.0f) - 10.0f, -(rand() % 1000) / 10.0f);
	}
